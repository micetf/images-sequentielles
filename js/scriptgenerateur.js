...
$(document).ready(function(){
  ...
  $('#images').change(function(){
    ...
    $('#sequence').empty().append('<td class="attendre">Patientez, chargement des images !</td>');
    ...
    var chargerImage=function(noImage) {
      ...
      reader.onload=function(event) {
        ...
        img.onload=function(){
          var ratio=Math.min(wImage/img.width,hImage/img.height);
          var wDelta=Math.floor(wImage-this.width*ratio)/2;
          var hDelta=Math.floor(hImage-this.height*ratio)/2;
          ctx.drawImage(this,0,0,this.width,this.height,wDelta,hDelta,this.width*ratio,this.height*ratio);
          imagesChargees++;
    ...
    for (var i=0; i<nbreImages; i++) {
      if (mesImages[i].type.match('image.*') && (typeof window.FileReader!=='undefined')) {
        chargerImage(i);
      }
    }
  ...
  $('#archive').click(function() {
    $('#donnees').empty();
    $('canvas').each(function(){
      $('#donnees').append('<input type="text" name="images[]" value="'+this.toDataURL()+'"/>');
    });
    return true;
  });
